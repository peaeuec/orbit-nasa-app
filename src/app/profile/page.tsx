...
export default async function ProfilePage() {
  ...
  // We filter out nulls immediately
  const savedPostsRaw = await Promise.all(likedIds.map(id => getPostById(id)));
  const savedPosts = savedPostsRaw.filter((post): post is NonNullable<typeof post> => post !== null);

  // 4. Fetch Global Like Counts
  const likeCounts = await getBulkLikeCounts(likedIds);
  
  // Debug: Check your terminal to see if this prints numbers!
  console.log("Profile Likes Debug:", likeCounts); 
